...
export default function PaymentIn() {
 ...
 supabase.from('bill_payments').select('amount').eq('date', today),
 supabase.from('bill_payments').select('amount').gte('date', startOfMonth),
 supabase.from('bills').select('balance_due').gt('balance_due', 0).neq('status', 'cancelled')
 ]);
 ...
 *,
 ...
 .ilike('payment_number', 'PAY-%')
 .order('created_at', { ascending: false })
 .limit(1);
 
 if (data && data.length > 0 && data[0].payment_number) {
 const lastNo = parseInt(data[0].payment_number.replace('PAY-', ''), 10);
 if (!isNaN(lastNo)) {
 setFormData(prev => ({ ...prev, paymentNumber: `PAY-${String(lastNo + 1).padStart(3, '0')}` }));
 return;
 }
 }
 setFormData(prev => ({ ...prev, paymentNumber: `PAY-001` }));
 };
 ...
 const amtReceived = Number(formData.amount);
 if (amtReceived <= 0) return setError('Amount must be greater than 0');
 ...
 const { error: insertErr } = await supabase.from('bill_payments').insert(paymentsToInsert);
 ...
 current_balance: Number(partyData.current_balance || 0) - amtReceived
 }).eq('id', selectedParty);
 }
 ...
 <p className="text-surface-500 text-sm">No unpaid bills found for this party.</p>
 </div>
 ) : (
 ...
 <div className="animate-fade-in w-full">
   ...
   <p className="text-xs font-bold text-surface-500 mb-1 uppercase tracking-wider">Total Received Today</p>
   <h3 className="text-xl font-bold text-surface-900">₹ {fmt(summary.today)}</h3>
   </div>
   <div className="bg-white rounded-xl shadow-sm border border-surface-200 py-3 px-4">
   <p className="text-xs font-bold text-surface-500 mb-1 uppercase tracking-wider">Total Received This Month</p>
 ...
 <button className="text-primary-600 font-semibold border-b-2 border-primary-600 pb-2 px-1">Payment Received</button>
 </div>
