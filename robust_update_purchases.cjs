...
if (!content.includes('ActionMenu')) {
  content = content.replace(
    /HiDotsVertical\n\} from 'react-icons\/hi';/g,
    "HiDotsVertical,\n HiOutlinePencilAlt,\n HiOutlineDocumentDuplicate,\n HiOutlineReceiptRefund,\n HiOutlineBan,\n HiOutlineTrash\n} from 'react-icons/hi';\nimport ActionMenu from '../../components/ActionMenu';"
  );
  if (!content.includes('ActionMenu')) {
    // fallback if no newline before }
    content = content.replace(
      /HiDotsVertical \} from 'react-icons\/hi';/g,
      "HiDotsVertical, HiOutlinePencilAlt, HiOutlineDocumentDuplicate, HiOutlineReceiptRefund, HiOutlineBan, HiOutlineTrash } from 'react-icons/hi';\nimport ActionMenu from '../../components/ActionMenu';"
    );
  }
}

// 2. Add handlers if not present
if (!content.includes('handleCancelBill')) {
  const handlers = `
...
content = content.replace(
  /<td className="px-4 py-4 text-center text-gray-400">\s*<button className="p-1 hover:bg-gray-100 rounded-md transition-colors" onClick=\{[^{}]*\{[^}]*\}\}>\s*<HiDotsVertical className="w-5 h-5" \/>\s*<\/button>\s*<\/td>/g,
  '<td className="px-4 py-4 text-center"><ActionMenu options={buildMenuOptions(inv)} /></td>'
);
