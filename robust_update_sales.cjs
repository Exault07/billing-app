...
if (!content.includes('ActionMenu')) {
  content = content.replace(
    /HiOutlineDotsVertical,\s*\} from 'react-icons\/hi';/g,
...
content = content.replace(
  /<td className="py-3 px-4 text-center" onClick=\{e => e\.stopPropagation\(\)\}>\s*<button className="text-surface-400 hover:text-surface-800">\s*<HiOutlineDotsVertical className="w-5 h-5" \/>\s*<\/button>\s*<\/td>/g,
...
console.log('Updated SalesInvoices.jsx');
